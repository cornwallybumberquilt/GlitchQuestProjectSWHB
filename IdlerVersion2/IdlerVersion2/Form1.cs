namespace IdlerVersion2
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            _blip.GetAdjacentCells(_world._grid._squares);
            MakeGridVisible();
            // while (true) { Tick(); }
        ...
        public void MakeGridVisible()
        {
            ...
            for (int x = 0; x < _world._numRows; x++)
            {
                ...
                for (int y = 0; y < _world._numCols; y++)
                {
                    ...
                    _world._grid._squares[i]._x = x;
                    _world._grid._squares[i]._y = y;
                    // sets the visibility of the grid to true
                    _world._grid._squares[i].Visible = true;

                    this.Controls.Add(_world._grid._squares[i]);

                    // add the grid to the forms controls
                    Controls.Add(_world._grid._squares[i]);

                    this._world._grid._squares[i].Location = new Point(x * _world._cellSize, y * _world._cellSize);
                    i++;
                }
            }
